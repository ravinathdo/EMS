...
class Employee_model extends CI_Model {
    ...
    function getPositionList() {
     ...
     public function getEmpPossition($empid) {
        /*
          SELECT POSITION.description FROM POSITION INNER JOIN emp_position
          ON emp_position.position_id = POSITION.posi
          WHERE emp_position.employee_id = 1
        */
//        echo 'id:'.$empid;
        $this->db->select('POSITION.*');
        $this->db->from('POSITION');
        $this->db->join('emp_position','emp_position.position_id = POSITION.posi');
//        $where = " emp_position.employee_id = '".$empid."' ";
        $this->db->where('emp_position.employee_id', $empid);
//        $this->db->where($where);
        $query = $this->db->get();
